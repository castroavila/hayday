...
tab_pricing = html.Div(
    [
        html.Div(
            [
                html.Label('place:', style={'width': '8%'}),
                dcc.Dropdown(
                    id='place',
                    options=[
                        {'label': i, 'value': i} for i in df['place'].unique()
                    ],
                    value=['bakery'],
                    multi=True,
                    style={'width': '80%'},
                ...
                html.Button(
                    'Update', id='button', style={'align-items': 'left'}
                ),
            ],
            style={
                'widht': '100%',
        ...
        html.Div(
            [
                dcc.Checklist(
                    id='options',
                    options=[{'label': 'Every place', 'value': 'everyplace'}],
                    value=[],
                )
            ]
        ),
...
def update_plot(button, places):
    ...
    for place in places:
        ...
        tracer = go.Scatter(
            x=df_place['prod_cost'],
            y=df_place['profit_per_hour'],
            name=place,
...
@app.callback(
    [Output('image_item', 'src'), Output('message', 'children')],
...
@app.callback(Output('place', 'value'), Input('options', 'value'))
